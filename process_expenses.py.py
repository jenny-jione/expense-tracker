...
with open('./expense2024.csv', 'r') as f, open('./my-app/public/data.csv', 'w', newline='') as f2:
    ...
    for i in range(1, month+1):
        ...
        # sorted_data[i] = dict(sorted(data[i].items(), key=lambda x:x[1]))
        print(sorted_data[i])
        print()

    # TODO: 지금은 월별로 카테고리별 금액 내림차순으로 write했는데, 전체 금액 기준으로 내림차순으로 바꾸기.
    ...
    for cate, money in cate_rank_sorted.items():
        print(cate, money)
